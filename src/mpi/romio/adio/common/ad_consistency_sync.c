...
{
    int err;
    static char myname[] = "ADIOI_GEN_CONSISTENCYSYNC";
    ...
    *error_code = MPI_SUCCESS;
    ...
    if (fd->is_open > 0 && fd->dirty_write) {
        ...
        /* CHEN:
         * ADIOI_GEN_FLUSH uses POSIX fsync to 
         * move dirty data to storage device.
         * For consistency purpose, we do nothing,
         * as POSIX write() is strongly consistent.
         */
        //err = fsync(fd->fd_sys);
    ...
    /* CHEN:
     * we ignore this hint and require users
     * to impose temporal order
     */
    /*
